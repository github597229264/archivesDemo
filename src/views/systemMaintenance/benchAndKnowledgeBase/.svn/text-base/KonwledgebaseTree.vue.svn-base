<template>
	<div class="knowledegTreeContent">
	    <el-tree
	      ref="tree"
	      :data="treeList"
	      node-key="id"
...
<script>
	export default {
		...
		methods: {
			/**
			 * 监听树点击事件
			 * 回调父组件中comTreeCall事件，传回选中的对象
			 */
			treeSelected(data,node,self) {
		        let returnObj =data;
		        this.$emit('baseTreeCall',returnObj);	
		   },
            /**
             * 获取列表数据
             */
