<template>
	<div class="resume-con">
		...
		<!--表格-->
		<el-table ref="table" :height="tableHeight" @selection-change="handleSelectionChange" border :data="ResumeList" style="width: 100%">
			...
			<el-table-column prop="a1703" label="起始时间" show-overflow-tooltip width="300" align="center">
			</el-table-column>
			<el-table-column prop="a1704" label="截止时间 " show-overflow-tooltip width="300" align="center">
			</el-table-column>
			<el-table-column prop="a1701" label="内容" show-overflow-tooltip width="750" align="center">
			</el-table-column>
		...
		<el-dialog :title="dialog_title" width="65%" height="350px" :visible.sync="isShow" v-if='isShow' append-to-body :close-on-click-modal="false">
			<ResumeAdd :selectDeceive="chioceObj" :addOrEdit="addOrEdit" @backCall="getAddBack"></ResumeAdd>
		</el-dialog>
		<!--编辑弹出层-->
	</div>
</template>
...
<script>
	...
	export default {
		...
		components: {
			Pagination, //分页组件
			ResumeAdd, //扩充-简历信息-添加

		},
		data() {
			return {
				ResumeList: [], //扩充信息-简历信息查询列表
				pageObj: { //分页
		...
		methods: {
			...
			handleEdit(index, row) { //调动信息-编辑
				...
				this.isShow = true;
				this.chioceObj = this.ResumeList[index];
			},
			/*
			 * 删除用户
			 */
			...
			handleDelete() {
				...
				if(this.selectedId == '') {
					...
					this.$confirm('确定要删除选中记录吗?', '操作提示', {
						...
						this.$axios({
							method: 'get',
							url: this.API.deleteA17,
							params: {
								'a1700': this.selectedId,
							}
						}).then((res) => {
							if(res.data.code == "1") {
								this.getResumeList();
