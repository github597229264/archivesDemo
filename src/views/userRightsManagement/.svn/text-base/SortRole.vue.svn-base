<template>
	<div class="sort-roleManagement-container">
		<div class="sort-roleManagement-title">
		...
		<div class="sort-roleManagement-content">
			<!--表格-->
			<el-table class="material_table" ref="srotRoleTable" border style="width: 100%;" highlight-current-row :row-class-name="tableRowClassName" height="350px" :data="sortList" @row-click="clickRow">
				<el-table-column label="序号" align="center" width="60">
					<template slot-scope="scope"> {{scope.$index+1}}</template>
				</el-table-column>
				<el-table-column prop="name" label="名称" align="center">
				</el-table-column>
				<el-table-column prop="workbenchName" label="工作台ID" align="center">
				</el-table-column>
...
export default {
	name:"SortroleManagementContainer", //排序
	components: {},
	props:[""],
	...
	methods: {
		...
		restoreData(){
			...
			// 	loading:{
			// 		isShow:true,
			// 		target:'.sort-roleManagement-container'
			// 	},
			// 	method: 'get',
			// 	url: this.API.initializationRoleSort,
			// }).then((res) => {
			// 	var resData = res.data;
			// 	if(resData.code == "1") {
			// 		this.$message.success('排序成功！');
			// 		this.$emit('sortRoleCall',true);
			// 	} else {
			// 		this.$message({
			// 			type: 'error',
			// 			message: '排序失败，请刷新重试！'
...
<style lang="scss" scoped>
	.sort-roleManagement-container {
		height: 100%;
		.sort-roleManagement-title{ margin:15px 0px;}
		.sort-roleManagement-content{
			margin: 15px 0px;
		}
	}
</style>