	...
	<el-container class="sort-detail-container">
		...
		<el-main>
			...
			<div class="sort-detail-content">
				<!-- 表格 -->
				<el-table ref="table" class="table" :height="tableHeight" :data="sortTableList" style="width: 100%;height: 100%;" @row-click="clickRow" :row-class-name="tableRowClassName" highlight-current-row border>
					<el-table-column type='index' label="序号" align="center" width="80"></el-table-column>
					<el-table-column prop="name" label="菜单" align="center"></el-table-column>
		...
		methods: {
			...
			getData() {//个人菜单排序列表页
				...
				this.$axios({
					...
					methods: 'get',
					url: this.API.sortMyUserMenusList,
					params: {
						menuid:this.menuid
					}
				}).then((res) => {
					...
					console.log(res);
					if(resData.code == "1") {
						this.sortTableList = resData.data;
						console.log(this.sortTableList);
					} else {
						this.$message({
							type: "error",
			...
			initSort() { // 初始化排序
				this.$axios({
					...
					methods: 'get',
					url: this.API.initialOrderMyUserMenus
				}).then((res) => {
					var resData = res.data;
					if(resData.code == "1") {
						this.getData();
