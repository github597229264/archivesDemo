<template>
	<div>
		...
		<div class='main' >
			<div class="l leftAside" ref='leftBox' :style="{height:leftHeight}">
				<!--全文检索-->
				...
				<div class="flex-bottom">
					<!--收藏快查-->
					<div class="archives-search">
						<el-button type="primary" @click="collectSearch">收藏快查</el-button>
					</div>
					<!--查询日志-->
					<div class="archives-search">
						<el-button type="primary" @click="getQueryLogs">查询日志</el-button>
					</div>
				</div>
			</div>
			<div class="contentRight r">
				<div class="left">
					<el-button type="primary" @click='getCollection'>收藏</el-button>
		...
		<!--收藏快查-->
		<el-dialog title="收藏快查" width="70%"  :visible.sync="collect_search"  v-if='collect_search' :close-on-click-modal="false" append-to-body>
			<collectAndSearch  @customBack="getAddBack"></collectAndSearch>
		</el-dialog>
		<!--收藏-->
		<el-dialog title="档案收藏" width="50%"  :visible.sync="collect_show"  v-if='collect_show' :close-on-click-modal="false" append-to-body>
			<collection  @customBack="getAddBack"></collection>
		</el-dialog>
		<!--查询日志-->
		<el-dialog title="查询日志" width="50%"  :visible.sync="query_logs"  v-if='query_logs' :close-on-click-modal="false" append-to-body>
			<queryLogs  @customBack="getAddBack"></queryLogs>
		</el-dialog>
	</div>
</template>
<script>
	...
	import collectAndSearch from './collectAndSearch.vue';
	...
	export default {
		...
		data() {
			return {
				...
				collect_search:false,//收藏快查
				edit_personal: false, //编辑回显
				collect_show:false,//收藏
				query_logs:false,//查询日志
				chioceObj: {}, //选中的数组
				retrievalData: [], //档案集合
				tableHeight:'',//表格高度
				leftHeight:'',//左侧高度
		...
		mounted() {
			...
			this.leftHeight = window.innerHeight - this.$refs.leftBox.offsetTop - 70 -40 +'px';
		},
		methods: {
			all_search() { //全文检索
