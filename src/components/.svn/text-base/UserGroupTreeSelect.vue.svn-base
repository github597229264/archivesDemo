...
<script>
	...
	export default {
		name: 'UserGroupTreeSelect', //用户组树形下拉框
		props:[
		...
		methods: {
			...
			handleChange(valArray) {
		        ...
		        this.$emit('UserGroupTreeSelectCall',returnObj);	
		   },
			/**
			 * 按照id查找数据
                ...
                this.$axios({
                    ...
                    if(resData.code =="1"){
                    	...
                    	this.$emit('UserGroupTreeSelectCall',returnObj);
			          	...
			          	self.$message({
				            type: 'error',
				            message: '获取用户组树形下拉框数据失败，请刷新重试！'
			          	});     
                    }
                }).catch((err) => {
                	console.log("获取用户组树形下拉框数据="+err);
                	self.$message({
			            type: 'error',
			            message: '请求异常，请检查网络！'
			        });  
