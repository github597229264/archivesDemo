<template>
	<div class="PoliticalOutlookTreeSelect">
...
<script>
	...
	export default {
		name: 'PoliticalOutlookTreeSelect', //查借阅类型下拉框
		...
		methods: {
			...
			changeChioceId(newV){
				this.chioceId = newV;
				this.$emit('PoliticalOutlookTreeSelectCall', newV);
			},
            /**
             * 获取列表数据
             */
            getData() {
                this.$axios({
                	...
                	params:{
	                	code:this.API.constObj.CadreCodeDetailList[3].id//政治面貌
                	}
                }).then((res) => {
                    ...
                    if(resData.code =="1"){
                    	...
                    	if(this.chioceOptionId && this.chioceOptionId.length > 0){
                    		this.chioceId = this.chioceOptionId;
                    		this.$emit('PoliticalOutlookTreeSelectCall', this.chioceId);
                    	}
                    }else{
			          	this.$message({
				            type: 'error',
				            message: '获取政治面貌下拉框数据失败，请刷新重试！'
			          	});     
                    }
                }).catch((err)=>{
                	console.log("获取政治面貌下拉框数据="+err);
                	this.$message({
			            type: 'error',
			            message: '请求异常，请检查网络！'
			        });  
