...
<script>
	...
	export default {
		name: 'MenuTreeSelect', //菜单树形下拉框--返回object
		props:[
		...
		methods: {
			...
			handleChange(valArray) {
		        ...
		        this.$emit('MenuTreeSelectCall',returnObj);	
		   },
			/**
			 * 按照id查找数据
                ...
                this.$axios({
                    ...
                    if(resData.code =="1"){
                    	...
                    	this.$emit('MenuTreeSelectCall',returnObj);
			          	...
			          	self.$message({
				            type: 'error',
				            message: '获取菜单树形下拉框数据失败，请刷新重试！'
			          	});     
                    }
                }).catch((err) => {
                	console.log("获取菜单树形下拉框数据="+err);
                	self.$message({
			            type: 'error',
			            message: '请求异常，请检查网络！'
			        });  
