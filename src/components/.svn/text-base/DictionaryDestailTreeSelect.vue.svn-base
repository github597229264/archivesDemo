<template>
	<div class="DictionaryDestailTreeSelect">
        <el-cascader
            v-model="selectedOptions"
            :options="treeList"
            placeholder="请选择父级字典项"
...
<script>
	...
	export default {
		name: 'DictionaryDestailTreeSelect', //数据字典项下拉框
		props: [
		    'defaultOptions',//选中的字典项
            'syscodeid' //字典id
        ],
		data() {
			return {
			...
			getData() {
				this.$axios({
                    ...
                    if(resData.code === "1"){
                        ...
                        //递归转换为tree格式数据
                        this.treeList = this.commonFun.toTreeDataNormal(resData.data,{childKey:'id',fatherKey:'pid'});
                        let returnObj = {};
