<template>
	<div class="CertificatesTreeSelect">
...
<script>
	...
	export default {
		name: 'CertificatesTreeSelect', //证件类型下拉框
		props: ['chioceOptionId'], //选中的证件类型id
		data() {
			return {
				dataList: [],//下拉框数据
				chioceId: '' //选中的id
		...
		methods: {
			...
			changeChioceId(newV){
				this.chioceId = newV;
				this.$emit('CertificatesTreeSelectCall', newV);
			},
            /**
             * 获取列表数据
             */
            getData() {
                this.$axios({
                	...
                	params:{
	                	code:this.API.constObj.CadreCodeDetailList[5].id//有效证件类型
                	}
                }).then((res) => {
                    ...
                    if(resData.code =="1"){
                    	...
                    	if(this.chioceOptionId && this.chioceOptionId.length > 0){
                    		...
                    		this.$emit('CertificatesTreeSelectCall', this.chioceId);
                    	}
                    }else{
			          	this.$message({
				            type: 'error',
