<template>
	<div class="MajorCategoryTreeSelect">
...
<script>
	...
	export default {
		name: 'MajorCategoryTreeSelect', //所学专业下拉框
		...
		methods: {
			...
			changeChioceId(id) {
				...
				this.$emit('MajorCategoryTreeSelectCall',id);
			},
			/**
			 * 获取列表数据
			 */
			getData() {
				this.$axios({
					...
					params: {
						code: this.API.constObj.CadreCodeDetailList[32].id //所学专业
					}
				}).then((res) => {
...
//						console.log(resData.data);
						if(this.chioceOptionId && this.chioceOptionId.length > 0){
                    		...
                    		this.$emit('MajorCategoryTreeSelectCall', this.chioceId);
                    	}
					} else {
						this.$message({
							type: 'error',
